/* ========================================
   Forest Theme — Portfolio Interactions
   ======================================== */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('scroll', () => {
        if (window.scrollY > 100) {
            nav.style.background = 'rgba(6, 13, 4, 0.9)';
        } else {
            nav.style.background = 'rgba(6, 13, 4, 0.7)';
        }
    }, { passive: true });
        ...
        btn.addEventListener('click', () => {
            ...
            galleryItems.forEach(item => {
                if (filter === 'all' || item.dataset.category === filter) {
                    ...
                    item.style.animation = 'fadeUp 0.45s forwards';
                } else {
    ...
    if (badge) {
        ...
        let i = 0;
        function typeChar() {
            if (i < text.length) { badge.textContent += text[i]; i++; setTimeout(typeChar, 45 + Math.random() * 25); }
        }
        setTimeout(typeChar, 850);
    }
    ...
    console.log('%c Elena Matyushenko %c Forest Theme ', 'background: #1a3d0c; color: #d8a0aa; font-size: 14px; font-weight: bold; padding: 4px 8px; border-radius: 4px 0 0 4px;', 'background: #0d1a08; color: #c98090; font-size: 14px; padding: 4px 8px; border-radius: 0 4px 4px 0; border: 1px solid #2a4a1a;');
});
