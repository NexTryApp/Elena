...
/* ========================================
   ...
   Based on Pavel Dobryakov's fluid sim
   Modified: Orange/Ginger color palette
   ======================================== */
...
function addKeywords(source, keywords) {
    ...
    let keywordsStr = '';
    keywords.forEach(kw => { keywordsStr += '#define ' + kw + '\n'; });
...
const bloomFinalShader = compileShader(gl.FRAGMENT_SHADER, `
    ...
    void main () {
        gl_FragColor = vec4(intensity * texture2D(uTexture, vUv).rgb, 1.0);
    }
`);
...
// Dithering texture (generated procedurally instead of loading external file)
...
// Compile display shader with keywords
let displayMaterial;
function updateDisplayMaterial() {
    let keywords = [];
    if (config.BLOOM) keywords.push('BLOOM');
    if (config.SUNRAYS) keywords.push('SUNRAYS');
    displayMaterial = new GLProgram(
        baseVertexShader,
        compileShader(gl.FRAGMENT_SHADER, displayShaderSource, keywords)
    );
}
...
const bloomBlurProgram = new GLProgram(blurVertexShader, bloomBlurShader);
...
// ORANGE/GINGER color generation - bright glowing fire effect
function generateColor() {
    // Hue range: deep red to orange (0.0 - 0.09 in HSV)
    let h = Math.random() * 0.09;
    let s = 0.8 + Math.random() * 0.2; // Very high saturation
    let v = 0.8 + Math.random() * 0.2; // Very bright

    let c = HSVtoRGB(h, s, v);
    // High intensity for vivid glowing fluid
    c.r *= 0.45;
    c.g *= 0.45;
    c.b *= 0.45;
    return c;
...
// Input handling - listen on document so fluid follows mouse over all content
